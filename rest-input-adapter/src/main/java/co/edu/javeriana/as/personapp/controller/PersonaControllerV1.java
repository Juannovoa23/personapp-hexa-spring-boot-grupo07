...
public class PersonaControllerV1 {
	...
	public PersonaResponse obtenerPersona(@PathVariable String database, @PathVariable String dni) {
		log.info("Into obtenerPersona REST API");
		return personaInputAdapterRest.buscarPersona(database.toUpperCase(), dni);
	}

	@PutMapping(path = "/{database}", produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
	public PersonaResponse editarPersona(@PathVariable String database, @RequestBody PersonaRequest request) {
		log.info("Into editarPersona REST API");
		return personaInputAdapterRest.editarPersona(database.toUpperCase(), request);
	}

	@DeleteMapping(path = "/{database}/{dni}", produces = MediaType.APPLICATION_JSON_VALUE)
	public Boolean eliminarPersona(@PathVariable String database, @PathVariable String dni) {
